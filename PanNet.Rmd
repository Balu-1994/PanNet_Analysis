...
```{r K-Means,  fig.height = 6, fig.width = 8, echo=TRUE}
  ...
  # Compute k-means
  set.seed(123)
  res.km <- kmeans(scale(t(assay(vsd))), 2, nstart = 20)

plotKmeans <- function(norm_data) {
  

  
  # Dimension reduction using PCA
  res.pca <- prcomp(t(assay(norm_data)),  scale = TRUE)
  ...
  ind.coord$condition <- norm_data$condition
  ind.coord$row.names <- colnames(norm_data)
...
### PCA Plot with Kmeans Cluster

* Number of cluster - 2

```{r PCA with K-means,  fig.height = 4, fig.width = 5, echo=TRUE}
...
ggplot(pcaData, aes(PC1, PC2, color = KmeanCluster, shape = condition)) + geom_point(size = 3) + 
  geom_text_repel(aes(label = rep), size = 3, color = "black") +
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar[2],"% variance")) + 
  theme_classic()
